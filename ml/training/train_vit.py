"""
train_vit.py — Baseline A: Solo Imagen (ViT)
=============================================
Entrena un clasificador basado únicamente en ViT-Base/16.
Arquitectura: ViT → CLS token → MLP Head → 14 clases

Fases:
  Fase 1: ViT congelado, solo se entrena el MLP Head.
  Fase 2: Se descongelan las últimas 2 capas del ViT.
"""
...
from ml.models.vit_encoder import ViTEncoder
...
CONFIG = {
    ...
    "checkpoint_dir":    "ml/checkpoints/vit_baseline",
    "tokenizer_name":    "bertin-project/bertin-roberta-base-spanish",
    "batch_size":        8,
    "max_token_length":  128,
    "num_workers":       0,
    # Fase 1 (encoders congelados)
    "lr_head":           1e-4,
    "epochs_phase1":     10,
...
class ViTClassifier(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, freeze=True):
        super().__init__()
        self.vit = ViTEncoder(freeze_layers=freeze)
        hidden = 768
    ...
    def forward(self, pixel_values, **kwargs):
        ...
        tokens = self.vit(pixel_values)          # (B, 197, 768)
...
def run_phase(model, train_loader, val_loader, optimizer, scheduler, criterion,
    ...
    """Bucle de entrenamiento para una fase. Devuelve el historial de métricas."""
    best_val_f1  = 0.0
    no_improve   = 0
    history      = []
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  BOLÍVAR RESPONDE — Baseline A: Solo ViT")
    print(f"  Dispositivo: {device}")
    print(f"{'='*60}")

    # ── DataLoaders ───────────────────────────────────────────────────
    # El ViT no necesita texto, pero reutilizamos el mismo dataset.
    ...
    # ─────────────────────────────────────────────────────────────────
    # FASE 1: ViT congelado — solo entrenamos el MLP Head
    # ─────────────────────────────────────────────────────────────────
    model = ViTClassifier(freeze=True).to(device)
    ...
    hist1, best_f1_phase1, best_path1 = run_phase(
    ...
    # ─────────────────────────────────────────────────────────────────
    # FASE 2: Descongelar últimas 2 capas del ViT
    # ─────────────────────────────────────────────────────────────────
    ...
    print(f"\n{'='*60}")
    print(f"  RESULTADOS FINALES — Baseline A (ViT)")
